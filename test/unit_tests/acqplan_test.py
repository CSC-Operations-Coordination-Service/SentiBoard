...
from apps.ingestion.acq_plan_ingestor import (
    AcqPlanIngestor,
    select_acq_link_after,
    select_acq_link_before,
    select_acq_link_includes_after_n_days_past,
)
from apps.ingestion.acq_link_page import (
    SatelliteAcqPlanLink,
    AcqLinksTable,
    AcqPlanLinksPageParser,
)
...
class AcqPlanTestCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        acqplan_links = {"S1": AcqLinksTable(), "S2": AcqLinksTable()}
    ...
    def testAcqPlanLinkParser(self):
        ...
        self.assertEqual(
            "2023-05-26 17:40:00", slink_1.start_date.strftime("%Y-%m-%d %H:%M:%S")
        )

        slink_1 = SatelliteAcqPlanLink(
            "/d/sentinel/s4a_mp_user_20230512t174000_20230618t194000", "https://abc"
        )
        self.assertEqual(
            "/d/sentinel/s4a_mp_user_20230512t174000_20230618t194000", slink_1.ref_url
        )
        self.assertEqual(
            "2023-05-12 17:40:00", slink_1.start_date.strftime("%Y-%m-%d %H:%M:%S")
        )
    ...
    def load_page_links(self, mission, page, div_config, num_acqplans):
        acqparser = AcqPlanLinksPageParser(
            self.ref_url, page, div_config, self._acqplan_links[mission]
        )
        ...
        for sat, acqplan_satlist in acqplans.items():
            self.assertNotEqual(
                len(acqplan_satlist), 0, f"Exepected not empty Link List for sat {sat}"
            )
        return acqplans
    ...
    def load_links(self, mission, acqtype, satellites):
        ...
        div_cfg = {AcqPlanLinksPageParser.ACQPLAN_DIV_KEY: div_classes}
    ...
    def test_link_selection(self):
        ...
        acqparser = AcqPlanLinksPageParser(self.ref_url, html_page, div_cfg, acqlinks)
        acqparser.get_acqplan_link_urls()
        ...
        for sat, sat_links in selected_acq_plans.items():
            ...
            self.assertListEqual(
                list(sat_links),
                sorted(tosort_list, reverse=True, key=lambda x: x.start_date),
            )
    ...
    def test_download_links(self):
        ...
        with self.assertRaises(
            Exception, msg="Getting KML Fragments for old date did not raise exception"
        ) as raises_cm:
            ingestor.get_kml_fragments("S2", "S2A", ["2023-07-07"])
