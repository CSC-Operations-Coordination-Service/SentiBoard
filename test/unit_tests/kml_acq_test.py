...
from apps.ingestion.orbit_acquisitions import (
    AcquisitionPlanOrbitDatatakeBuilder,
    ORBIT_PROPAGATION_STEP,
)
...
class AcqPlanFragmentsTestCaseBase(unittest.TestCase):
    ...
    def manage_cache_config(cls, environment):
        config = read_config_file(cls.test_config_path, environment)["config"]
    ...
    def setUpClass(cls):
        cls.kml_filenames = [
            "S1A_MP_USER_20230607T174000_20230627T194000.kml",
        ...
        cls.s1_kml_aug_sep_filenames = [
            "S1A_MP_USER_20230907T174000_20230927T194000.kml",
            "S1A_MP_USER_20230908T174000_20230928T194000.kml",
            "S1A_MP_USER_20230912T174000_20231002T194000.kml",
        ]
        cls.s1_sep_filenames = [
    ...
    def setUp(self) -> None:
        ...
        logging.basicConfig(
            handlers=[
                logging.FileHandler(f"unittest_{test_case}.log"),
    ...
    def load_S1_kml_file(self, frag_mgr: AcqPlanFragments, kmlfile, expected_num):
        self.load_S1_kml_file_no_check(frag_mgr, kmlfile)
        self.assertEqual(
            expected_num,
            frag_mgr.num_fragments,
            f"Expected {expected_num} daily fragments",
        )

    def load_S2_kml_file(self, frag_mgr: AcqPlanFragments, kmlfile, expected_num):
    ...
    def load_S1_kml_file_no_check(self, frag_mgr: AcqPlanFragments, kmlfile):
        kml_fullpath = os.path.join(self.kml_path, kmlfile)
...
class AcqPlanFragmentsTestCase(AcqPlanFragmentsTestCaseBase):
    ...
    @time_machine.travel("2023-06-16" " 03:00 +0000")
    def test_kmlAcquisitionExtractDailyFolders(self):
        ...
        self.load_S1_kml_file(fragments_manager, self.kml_filenames[0], 17)
        for day_str in days_list:
            self.assertTrue(fragments_manager.get_fragment(day_str))

    @time_machine.travel("2023-06-21" " 03:00 +0000")
    def test_kmlS1AcquisitionOrderedPlacemarks(self):
        day = "2023-06-11"
        max_age = 10
        fragments_manager = AcqPlanFragments("kmltest", "S1A", max_age)
        # TODO: manage, mission, satellite, day
        self.load_S1_kml_file(fragments_manager, self.kml_filenames[0], max_age + 7)
    ...
    def test_kmlS1MultipleKmlOrderedPlacemarks(self):
        ...
        self.load_S1_kml_file(fragments_manager, self.kml_filenames[1], max_age + 9)
    ...
    @time_machine.travel("2023-07-01" " 03:00 +0000")
    def test_kmlS2AcquisitionOrderedPlacemarks(self):
        day = "2023-06-21"
        max_age = 10
        fragments_manager = AcqPlanFragments("kmltest", "S2A", max_age)
        # TODO: manage, mission, satellite, day
        self.load_S2_kml_file(fragments_manager, self.s2_kml_filenames[1], max_age + 3)
        day_fragment: AcqPlanDayFragment = fragments_manager.get_fragment(day)
        pm_list = day_fragment.placemark_list
    ...
    def test_kmlS2MultipleKmlOrderedPlacemarks(self):
        ...
        fragments_manager = AcqPlanFragments("kmltest", "S2A", max_age)
        # TODO: manage, mission, satellite, day
        self.load_S2_kml_file(fragments_manager, self.s2_kml_filenames[2], 19)
        day_fragment: AcqPlanDayFragment = fragments_manager.get_fragment(day)
        pm_list1 = day_fragment.placemark_list
        self.assertNotEqual(len(pm_list1), 0, "No Placemarks found")
        pm_names_dates1 = [(pm.name, pm.TimeSpan.end) for pm in pm_list1]
        # Expected num fragments includes previous number + new fragments
        self.load_S2_kml_file(fragments_manager, self.s2_kml_filenames[1], 19 + 8)
        day_fragment2: AcqPlanDayFragment = fragments_manager.get_fragment(day)
        pm_list2 = day_fragment2.placemark_list
    ...
    def test_kmlS2MultipleKmlNoDupPlacemarks(self):
        ...
        self.assertEqual(
            len(pm_names), len(pm_unique_names), "Placemark List has duplicates"
        )

    @time_machine.travel("2023-09-26" " 18:00 +0000")
    def test_kmlS1MultipleKmlNoDupPlacemarksSeptBug(self):
        day = "2023-09-26"
        max_age = 15
        fragments_manager = AcqPlanFragments("kmltest", "S1A", max_age)
        # TODO: manage, mission, satellite, day
        self.load_S1_kml_file_no_check(fragments_manager, self.s1_bug_kml_filenames[0])

        # Expected num fragments includes previous number + new fragments
        self.load_S1_kml_file_no_check(fragments_manager, self.s1_bug_kml_filenames[1])
        self.load_S1_kml_file_no_check(fragments_manager, self.s1_bug_kml_filenames[2])
        self.load_S1_kml_file_no_check(fragments_manager, self.s1_bug_kml_filenames[3])
        day_fragment: AcqPlanDayFragment = fragments_manager.get_fragment(day)
        ...
        # print(discriminator)
        duplicates = [
            (acq_id, acq_names_list)
            for acq_id, acq_names_list in discriminator.items()
            if len(acq_names_list) > 1
        ]
        print("Duplicati: ", duplicates)
        self.assertEqual(
            len(acq_list), len(acq_unique_ids), "Acquisition List has duplicates"
        )
    ...
    @time_machine.travel("2023-06-30" " 03:00 +0000")
    def test_kmlS2AcquisitionExtractDailyFolders(self):
        ...
        # TODO: manage, mission, satellite, day
        self.load_S2_kml_file(fragments_manager, self.s2_kml_filenames[0], 8)
        for day_str in days_list:
            self.assertTrue(fragments_manager.get_fragment(day_str))
    ...
    def test_kmlMergeDailyFolders(self, daily_fragments):
        ...
        self.load_S1_kml_file(fragment_table, self.kml_filenames[0], 21)
        # Load Kml 2 fragments
        self.load_S1_kml_file(fragment_table, self.kml_filenames[1], 23)
    ...
    def kml_getFragmentsForDay(self, day_str):
        ...
        # Load Kml 1 fragments
        self.load_S1_kml_file(older_fragment_table, self.kml_filenames[0], 21)
        # Load Kml 2 fragments
        newer_fragment_table = AcqPlanFragments("kmltest2", "S1A", max_age)
        self.load_S1_kml_file(newer_fragment_table, self.kml_filenames[1], 21)
    ...
    def test_kmlFolderMergeFirstPartialDay(self):
        ...
        print(
            "Duplicated dates: ",
            [name for name, count in Counter(placemark_names).items() if count > 1],
        )
        self.assertEqual(
            num_different_placemarks,
            num_placemakrs,
            f"Expected {num_different_placemarks}, found {num_placemakrs}",
        )
    ...
    def test_kmlAcquisitionBuildFromDailyFolders(self):
        ...
        self.load_S1_kml_file(fragment_table, self.kml_filenames[0], max_age + 7)
    ...
    def test_kmlLoadTwoFiles(self):
        ...
        self.load_S1_kml_file(fragment_table, self.kml_filenames[0], max_age + 7)
        self.load_S1_kml_file(fragment_table, self.kml_filenames[1], max_age + 9)
    ...
    def _generateKmlOneDay(self, day_str):
        ...
        self.assertTrue(
            schema_ogc.validate(generated_doc), "Generated KML was not valid for OGC"
        )
...
def _retrieve_elastic_datatakes(datatakes_path, json_file):
    ...
    json_path = os.path.join(datatakes_path, json_file)
...
class AcqPlanFragmentsCompletenessTestCase(AcqPlanFragmentsTestCaseBase):
    ...
    def _setup_datatakes_from_Json(self, json_file):
        json_path = os.path.join(self.datatakes_path, json_file)
    ...
    def _kmlS1DatatakePartial(self, day, dt_id):
        ...
        fragments_manager = AcqPlanFragments("kmltest", "S1A", max_age)
        # TODO: manage, mission, satellite, day
        self.load_S1_kml_file(fragments_manager, self.s1_kml_aug_sep_filenames[2], 21)
        self.load_S1_kml_file(fragments_manager, self.s1_kml_aug_sep_filenames[1], 25)
        satellite = "S1A"
        ...
        self.assertEqual(dict, type(day_datatakes), "Day datatakes is not a Dict")
        print(
            "Datatakes satellites for day ", acq_day, ": ", list(day_datatakes.keys())
        )
        ...
        FragmentCompletenessHandler._load_datatake_completeness_on_placemarks(
            day_sat_datatakes, day_fragment, satellite, datatake_id_key, id_decoder
        )
        ...
        self.assertTrue(
            dt_id in day_sat_datatakes.keys(),
            f"No Datatatake found from Elastic with id {dt_id}",
        )
        ...
        for pm in one_fragment.placemark_list:
            ...
            for data_rec in pm.ExtendedData.Data:
                print(
                    "Data Record with name  ",
                    data_rec.attrib["name"],
                    "has value ",
                    data_rec.value.text,
                )
            acq_dt_status = acq_dt.get_data_record(acq_dt.PUB_STATUS_LABEL)
            self.assertTrue(
                acq_dt_status.value.text.startswith(
                    DatatakeCompleteness.PARTIAL_STATUS
                ),
                "Acquistiion Placemark has not Partial Status",
            )

    @time_machine.travel("2023-09-15" " 03:00 +0000")
    ...
    def test_kmlDatatakeCompletenessSteps(self):
        ...
        completenessHandler = FragmentCompletenessHandler(
            mission, fragments_table, daily_datatakes
        )
        mission_id_hnd = MissionDatatakeIdHandler(mission)
        # self.datatake_id_decoder = mission_id_hnd.datatake_id_decoder
        datatake_id_key = mission_id_hnd.datatake_id_key
        ...
        for acq_day in fragment_days:
            ...
            self.assertIsNotNone(
                day_datatakes, f"Not found datatakes for day {acq_day}"
            )
            self.assertEqual(dict, type(day_datatakes), "Day datatakes is not a Dict")
            print(
                "Datatakes satellties for day ",
                acq_day,
                ": ",
                list(day_datatakes.keys()),
            )
            day_sat_datatakes = day_datatakes.get(satellite)
            print(
                "S1 Datatake IDS for day ", acq_day, ":", list(day_sat_datatakes.keys())
            )
            id_decoder = mission_id_hnd.datatake_id_decoder
            completenessHandler._load_datatake_completeness_on_placemarks(
                day_sat_datatakes, day_fragment, satellite, datatake_id_key, id_decoder
            )
            # TODO: Check taht pmocleteness has been loaded on KML fragments
        one_fragment: AcqPlanDayFragment = fragments_manager.get_fragment(day)
        ...
        self.assertTrue(
            any(
                [
                    _pmdata.attrib["name"].startswith("Acquisition")
    ...
    def test_kmlDatatakeCompleteness(self):
        ...
        # TODO: manage, mission, satellite, day
        self.load_S1_kml_file(sat_fragments_manager, self.kml_filenames[0], 17)
    ...
    def test_kmlDatatakeCompletenessStyle(self):
        ...
        # TODO: manage, mission, satellite, day
        self.load_S1_kml_file(fragments_manager, self.kml_filenames[0], 21)
        satellite = "S1A"
    ...
    def test_setup(self):
        self._retrieve_elastic_datatakes(self.datatakes_path, self.Datatakes_S1S2_JSON)
    ...
    def _retrieve_elastic_S1_sept_datatakes(self):
        ...
        json_path = os.path.join(
            self.datatakes_path, self.Datatakes_S1_EndSeptember_JSON
        )
        with open(json_path, "w") as json_o:
            json.dump(
                dt_interval,
...
class GeoOperationsTestCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.manage_cache_config("config-test.json")
        logging.basicConfig(
            handlers=[logging.FileHandler("unittest.log"), logging.StreamHandler()],
            level=logging.DEBUG,
            format=" %(asctime)s - %(filename)s: %(lineno)s :: %(funcName)20s - %(levelname)s :: %(message)s",
        )
        ...
        self.step = 60
        self.propagator = OrbitPropagatorSkyfield(
            tle_data, step=self.step, extra_point=False
        )
    ...
    def test_point_between_east_west(self):
        ...
        self.assertTrue(
            self._value_included(point1.longitude.radians, west_trg.lon, east_trg.lon),
            "Target east/west have not longitude around start point",
        )
        self.assertTrue(
            self._value_included(point1.latitude.radians, west_trg.lat, east_trg.lat),
...
class OrbitAcqPlanTestCase(AcqPlanFragmentsTestCaseBase):
    ...
    def _retrieve_elastic_S3_datatakes(self, fromd, to, outfile):
        ...
        # compute_datatakes_completeness_status(dt_interval)
        # Save on JSON FILE Datatakes_S1S2_JSON
        json_path = os.path.join(self.datatakes_path, outfile)
        with open(json_path, "w") as json_o:
            json.dump(
                dt_interval,
                json_o,
    ...
    def test_S3_setup(self):
        ...
        self._retrieve_elastic_S3_datatakes(
            "2023-09-26", "2023-10-20", self.Datatakes_S3_October_JSON
        )
    ...
    def test_point_list_string(self):
        ...
        # Check result
        ref_str = (
            "85.73000,43.50000,0 145.14300,-13.87000,20.1 -121.98000,76.43000,760.86"
        )
        self.assertEqual(
            ref_str, coord_str, "Generated coordinates string not matching"
        )
    ...
    def test_build_line_placemark(self):
        ...
        print(
            "Created Placemark with name ",
            pm.name,
            " from datatake with id ",
            acq_datatake.datatake_id,
        )
        ...
        self.assertEqual(
            pm.TimeSpan.begin,
            datetime.strftime(acq_datatake.start_time, INTERVAL_TIME_FMT),
        )
    ...
    def test_build_line_fragment_fake_points(self):
        ...
        acq_day = "2023-09-03"
        mission_id_hnd = MissionDatatakeIdHandler(mission)
        fragm_builder = OrbitAcquisitionKmlFragmentBuilder(acq_day, satellite)
    ...
    def test_tle_two_days_datatakes_to_fragments(self):
        ...
        orbit_acq_builder = OrbitAcquisitionsBuilder(
            satellite, tle_data, step, acquisition_builder
        )
        for acq_day in acq_days:
            ...
            for dt_id, dt in sat_datatakes.items():
                ...
                acq.acquisition_points = orbit_acq_builder.compute_acquisition_points(
                ...
                sat_kml_builder.add_to_daily_folder(acq_day, satellite, acq_placemark)
            # Perform Checks on result, by reading the fragment
            fragment = sat_kml_builder.fragment
            self.assertEqual(len(sat_datatakes), len(sat_kml_builder.placemark_list))
    ...
    def test_orbit_time_interval_to_polygon(self):
        ...
        test_tle_1 = "S3A_20231012.tle"
        tle_data = self.tle_retriever.get_tle_data(test_tle_1)
        orbit_propagator = OrbitPropagatorSkyfield(tle_data, step, extra_point=True)
        # Instantiate a Orbit Acquisition Builder
    ...
    @time_machine.travel("2023-10-12" " 03:00 +0000")
    ...
    def test_satellite_acq_from_orbit_datatakes(self, mock_tle):
        ...
        past_num_days = 8
        mission_fragments = {
            sat: AcqPlanFragments(sat, sat, past_num_days) for sat in ["S3A"]
        }
        ...
        #
        # AcquisitionPlanOrbitDatatakeBuilder
        acq_builder = AcquisitionPlanOrbitDatatakeBuilder(
        ...
        self.assertEqual(len(day_fragment.placemark_list), len(s3a_day_datatakes))
    ...
    def test_extract_polygon_points_orbit_datatakes(self, mock_tle):
        ...
        daily_datatakes[day][satellite] = {
            dt["datatake_id"]: dt for dt in sliced_dt_list
        }
        # Configure the mock to return a value using the local mock function.
        ...
        propagator = OrbitPropagatorSkyfield(
            tle_data, ORBIT_PROPAGATION_STEP, extra_point=True
        )
