...
// shaded Earth + graticule, three orbit tracks with moving satellites, ground
// stations and pulsing datatake markers. Drag to rotate, wheel/pinch to zoom,
// click a datatake (or the list) to inspect it; a sim-clock plays/pauses/scrubs.
const D = Math.PI / 180;
const ORBITS = [
...
export default function AcquisitionGlobe({ stations, datatakes }: { stations: Station[]; datatakes: AcqDatatake[] }) {
  ...
  const st = useRef({ W: 0, H: 0, R: 0, baseR: 0, cx: 0, cy: 0, yaw: 0, tilt: -0.42, t: 0, zoom: 1, autoSpin: true, dragging: false, lastX: 0, lastY: 0, moved: false, simMs: Date.UTC(2026, 6, 16, 11, 4, 22), pinch: 0, targetYaw: 0, targetTilt: -0.42, flying: false });

  useEffect(() => { setSel(0); selRef.current = 0; }, [datatakes]);

  // Selecting a datatake (from the list or by clicking a marker) rotates the globe
  // so that datatake faces the viewer and centres — so a far-side selection still
  // reveals itself instead of staying hidden behind the globe.
  const select = (i: number) => {
    setSel(i); selRef.current = i;
    const a = datatakes[i]; const s = st.current;
    s.targetYaw = a.lon * D;
    s.targetTilt = Math.max(-1.45, Math.min(1.45, a.lat * D));
    s.flying = true; s.autoSpin = false;
  };
  ...
  useEffect(() => {
    ...
    function frame() {
      ...
      if (!reduce && s.flying) {
        let dyaw = s.targetYaw - s.yaw; dyaw = Math.atan2(Math.sin(dyaw), Math.cos(dyaw)); // shortest angular path
        s.yaw += dyaw * 0.12; s.tilt += (s.targetTilt - s.tilt) * 0.12;
      ...
      ctx.lineWidth = 1; ctx.strokeStyle = "rgba(54,208,224,0.35)"; ctx.beginPath(); ctx.arc(cx, cy, R, 0, 6.2832); ctx.stroke();
      for (let la = -60; la <= 60; la += 30) { const ring = []; for (let lo = 0; lo <= 360; lo += 5) ring.push(proj(la, lo)); strokePath(ring, la === 0 ? "rgba(54,208,224,0.22)" : "rgba(120,150,190,0.12)", la === 0 ? 1.2 : 1); }
      ...
      // Continents: project each land ring and stroke its coastline (visible
      // hemisphere only — strokePath breaks the path where z<=0). A soft wide pass
      // reads as landmass; a crisp thin pass draws the coastline on top.
      for (const ring of LAND) {
        const pts = ring.map(([lon, lat]) => proj(lat, lon));
        strokePath(pts, "rgba(88,120,150,0.22)", 3.2);
        strokePath(pts, "rgba(150,182,168,0.62)", 1);
      }
      ...
      datatakes.forEach((a, i) => {
        const p = proj(a.lat, a.lon); if (p.z > 0) {
          const col = a.cls === "ok" ? "#3DD68C" : a.cls === "warn" ? "#F5B544" : "#FF5C6C";
          ...
          if (isSel) {
            ...
            ctx.setLineDash([4, 5]); ctx.lineDashOffset = -s.t * 0.6;
      ...
      <div className="globe-card">
        <div className="globe-stage">
          ...
          <div className="simbar">
            ...
            <div className="simtime"><span ref={clockRef}>2026-07-16 11:04:22Z</span><small>SIMULATION TIME</small></div>
