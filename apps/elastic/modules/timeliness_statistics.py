...
def timeliness_convert_to_hour(timeliness_result):
    ...
    for key in timeliness_stats:
        if key == "count":
            continue
        if isinstance(timeliness_stats[key], dict):
            subdict = timeliness_stats[key]
            for subkey in subdict:
                subdict[subkey] = _safe_to_hours(subdict[subkey])
        else:
            timeliness_stats[key] = _safe_to_hours(timeliness_stats[key])
...
def _safe_to_hours(value):
    try:
        if value is None:
            return 0
        return float(value) / 1000000 / 3600
    except (TypeError, ValueError):
        logger.warning(
            "[Timeliness] Cannot convert value to hours: %s (type=%s)",
            value,
            type(value),
        )
        return 0
