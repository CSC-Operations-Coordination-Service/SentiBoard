...
def get_cds_archive_size_by_mission(start_date, end_date, mission):
    ...
    for satellite in mission_satellites.get(mission):
        results.extend(
            _get_cds_archive_size_by_satellite(
                start_date, end_date, mission, satellite, archive_levels
            )
        )
    logger.debug(
...
def _get_cds_archive_size_by_satellite(
    start_date, end_date, mission, satellite, levels
):
    ...
    if len(levels):
        level_condition = {"terms": {"product_level": levels}}
    else:
        level_condition = {{"term": {"product_level": levels[0]}}}
    ...
    query = {
        "bool": {
            "must": [
                ...
                level_condition,
                {"term": {"satellite_unit": satellite}},
                {"term": {"service_type": "LTA"}},
            ]
        }
    }
    ...
    aggs = {
        "group_by_level": {
            ...
            "aggs": {"total_size": {"sum": {"field": "content_length"}}},
        }
    }
    ...
    try:
        ...
        logger.debug(
            f"CDS LONG TERM ARCHIVE - Query for mission {mission} start: {start_date}, end: {end_date}: {query}"
        )
        query_start_time = perf_counter()
        level_data = elastic.search(index=index, query=query, aggs=aggs, size=0)[
            "aggregations"
        ...
        logger.error(
            "Failure of Long Term Archive Elastic query for mission %s, start: %s, end: %s",
            mission,
            start_date,
            end_date,
        )
