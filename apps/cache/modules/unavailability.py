...
def load_unavailability_cache_last_quarter():
    ...
    for dt in sat_unav_last_quarter:
        unav_stop = datetime.strptime(
            dt["_source"]["start_time"], "%Y-%m-%dT%H:%M:%S.%fZ"
        )
    ...
    logger.info(
        f"[END] Loading Sat Unavailability Cache in the last quarter - Execution Time : {cache_end_time - cache_start_time:0.6f}"
    )
...
def load_unavailability_cache_previous_quarter():
    ...
    cache_end_time = perf_counter()
    logger.info(
        f"[END] Loading Sat Unavailability Cache in the previous quarter - Execution Time : {cache_end_time - cache_start_time:0.6f}"
    )
