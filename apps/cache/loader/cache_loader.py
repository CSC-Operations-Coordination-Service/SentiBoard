...
SENTINEL_START_TIME = datetime.strptime("2014-01-01 00:00:00", "%Y-%m-%d %H:%M:%S")
...
class RestCacheLoader:
    ...
    def load_cache(self, period_id):
        ...
        """
        logger.info(
            "[BEG] Loading %s Cache for last %s", self._statistics_id, period_id
        )
    ...
    def load_cache_previous_quarter(self):
        logger.info(
            "[BEG] Loading  %s  Cache  for previous quarter", self._statistics_id
        )
        period_id = PREV_QUARTER
        cache_start_time = perf_counter()
        # Retrieve data takes in the last 3 months and store results of query in cache
        start_date, end_date = date_utils.prev_quarter_interval_from_date(
            datetime.today()
        )
        self._load_product_statistics_cache(period_id, start_date, end_date)
        cache_end_time = perf_counter()
    ...
    def load_cache_lifetime(self):
        logger.info("[BEG] Loading  %s  Cache  for lifetime", self._statistics_id)
        period_id = LIFETIME
        cache_start_time = perf_counter()
        # Retrieve data takes in the last 3 months and store results of query in cache
    ...
    def load_all_periods_cache(self):
        ...
        """
        logger.info("[BEG] Loading %s Cache for All periods", self._statistics_id)
        periods = [PeriodID.DAY, PeriodID.WEEK, PeriodID.MONTH, PeriodID.QUARTER]
        for period in periods:
            self.load_cache(period)
        logger.info(f"[END] Loading {self._statistics_id} Cache for all periods")

    def _load_product_statistics_cache(self, period_id, start_date, end_date):
        logger.debug(
            "Setting %s Cache for period: %s, from %s, to %s",
            self._statistics_id,
            period_id,
            start_date,
            end_date,
        ...
        logger.debug(
            "Loading period %s: from %s to %s", period_id, start_date_str, end_date_str
        )
        ...
        logger.debug(
            "Saving on cache for last %s publication of type: %s, with cache key: %s, timeout: %d",
            period_id,
            self._statistics_id,
            rest_api_prefix,
            seconds_validity,
    ...
    def _retrieve_statistics_data(self, period_id, start_date, end_date):
        ...
        for mission in self.mission_list:
            logger.debug(
                "Loading Publication Cache for period last %s - mission %s",
                period_id,
                mission,
            )
        ...
        publication_period_data = {
            "period": period_id,
            "interval": {"from": start_date.isoformat(), "to": end_date.isoformat()},
            "data": period_data,
        }
...
class RestCache_TrendLoader(RestCacheLoader):
    ...
    def _retrieve_statistics_data(self, period_id, start_date, end_date):
        logger.debug(
            "Retrieving Trend Data for period %s - from %s to %s",
            period_id,
            start_date,
            end_date,
        )
        ...
        if period_id in (PeriodID.QUARTER, PREV_QUARTER):
            ...
            trend_start_date = date_utils.get_date_before(
                trend_end_date,
                border_date=start_date,
                num_days=date_utils.WEEK_DAYS,
                pass_border=True,
            )
            logger.debug(
                "Trend interval from %s to %s, for date interval %s to %s",
                trend_start_date.isoformat(),
            ...
            logger.debug(
                "Number of days in Week based interval: %d", date_diff.days + 1
            )
            logger.debug(
                "Requested number of subperiods for quarter: %d", num_subperiods
            )
            # Compute actual number of subperiods, based on recomputed interval
            num_subperiods = int((date_diff.days + 1) / date_utils.WEEK_DAYS)
            logger.debug(
                "Computed number of subperiods for quarter: %d", num_subperiods
            )
        ...
        for mission in self.mission_list:
            ...
            logger.debug(
                "Using %d subperiods from %s to %s",
                num_subperiods,
                trend_start_date.isoformat(),
                trend_end_date.isoformat(),
            )
            ...
            mission_service_data = self._mission_elastic_fun(
                trend_start_date, trend_end_date, mission, num_subperiods
            )
            logger.debug(
                "Collecting results from Elastic Function %s",
                self._mission_elastic_fun.__name__,
            )
            for service, mission_data in mission_service_data["service_trend"].items():
                logger.debug(
                    "Saving to cache for mission %s, service %s, data: %s",
                    mission,
                    service,
                    mission_data,
                )
            ...
            logger.debug(
                "After loading Mission %s, global period Data is %s",
                mission,
                period_service_data,
            )
        subperiods = date_utils.get_interval_subperiods(
            trend_start_date, trend_end_date, int(num_subperiods)
        )
