...
def save_instant_messages(
    title, text, link, publication_date, message_type, modify_date=datetime.now()
):
    try:
        instant_message = InstantMessages(
...
def update_instant_messages(
    ...
    try:
        ...
        instant_message = (
            ...
            .filter(InstantMessages.id == message_id)
            .first()
        )
        if not instant_message:
            logger.warning("News with id %s not found", message_id)
            return None
        ...
        instant_message.messageType = message_type
...
def update_instant_messages_categorization(
    message_id,
    title,
    text,
    link,
    ...
    try:
        ...
        db.session.rollback()
        logger.exception(
            "Error updating instant message categorization: %s", message_id
        )
    return None
...
def get_instant_messages(start_date=None, end_date=None):
    try:
        if start_date is None or end_date is None:
            return InstantMessages.query.order_by(
                InstantMessages.publicationDate.asc()
            ).all()
        else:
            return (
                InstantMessages.query.filter(
                    InstantMessages.publicationDate is not None
                )
                .filter(InstantMessages.publicationDate >= start_date)
                .filter(InstantMessages.publicationDate <= end_date)
                .order_by(InstantMessages.publicationDate.asc())
                .all()
            )
    except Exception as ex:
...
def delete_instant_messages_by_id(uuid):
    try:
        db.session.query(InstantMessages).filter(
