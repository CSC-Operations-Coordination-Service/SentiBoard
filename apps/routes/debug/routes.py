...
mission_pub_extra_must_criteria = {
    ...
    "S2": [{"terms": {"product_level": ["L1C", "L2A"]}}],
}

mission_pub_extra_must_not_criteria = {
    "S3": [{"terms": {"product_level": ["L0_", "null"]}}],
    "S2": [{"term": {"product_type": "OLQC_REPORT"}}],
}
...
@blueprint.route(
    "/api/debug/cds-publication/<start_date>/<end_date>/<platform>", methods=["GET"]
)
@login_required
@flask_cache.cached(timeout=60 * 60)
def get_cds_publication_by_mission(start_date, end_date, platform):
    logger.info(
        "Requested Publications from %s to %s for  platform %s",
        start_date,
        end_date,
        platform,
    )
    ...
    try:
        ...
        start_date_str, end_date_str = elastic.date_interval_to_elastic_range(
            start_date, end_date
        )
        logger.info(
            "Querying publications for platform %s from %s to %s excluded",
            platform,
            start_date_str,
            end_date_str,
        )
        ...
        query_dict = {
            "bool": {
                ...
                "must_not": [{"term": {"product_level": "L__"}}],
            }
        }
        ...
        search_body = {
            ...
            "sort": [{"publication_date": "asc"}, {"_id": "asc"}],
        }
        ...
        for index in indices:
            ...
            try:
                ...
                result = elastic.get_connection().search(index=index, body=search_body)[
                    "hits"
                ]["hits"]
                ...
                if remove_dups:
                    ...
                    result_no_dup_names = {
                        result_rec["_source"]["name"]: result_rec
                        for result_rec in result
                        if mission in fname_ends
                        and result_rec["_source"]["name"].endswith(fname_ends[mission])
                    }
                ...
                while after_result is not None and len(after_result):
                    ...
                    after_result = elastic.get_connection().search(
                        index=index, body=search_after_body
                    )["hits"]["hits"]
                    if after_result is not None:
                        index_records += len(after_result)
                        logger.info(
                            "adding %d results; total now: %s",
                            len(after_result),
                            index_records,
                        )
                        if remove_dups:
                            result_no_dup_names = {
                                ...
                                if mission in fname_ends
                                and result_rec["_source"]["name"].endswith(
                                    fname_ends[mission]
                                )
                            }
                    ...
                    if index_records > max_records:
                        logger.warning(
                            "Collected more than 100.000 records. Interrupting"
                        )
                        break
...
@blueprint.route(
    "/api/debug/cds-product-timeliness/<start_date>/<end_date>/<mission>",
    methods=["GET"],
)
@blueprint.route(
    "/api/debug/cds-product-timeliness/<start_date>/<end_date>/<mission>/<timeliness>",
    methods=["GET"],
)
@blueprint.route(
    "/api/debug/cds-product-timeliness/<start_date>/<end_date>/<mission>/<timeliness>/<published>",
    methods=["GET"],
)
@login_required
@flask_cache.cached(timeout=60 * 60)
def get_cds_product_timeliness(
    start_date, end_date, mission, timeliness=None, published=True
):
    try:
        if not auth_utils.is_user_authorized():
            return Response(
        ...
        if timeliness is None:
            results = elastic_timeliness.get_cds_mission_product_timeliness(
                start_date, end_date, mission
            )
        else:
            results = elastic_timeliness.get_cds_product_timeliness(
                start_date, end_date, mission, timeliness, published
            )
        return Response(json.dumps(results), mimetype="application/json", status=200)
    except Exception as ex:
...
@blueprint.route(
    "/api/debug/cds-product/<start_date>/<end_date>/<datatake_id>", methods=["GET"]
)
@login_required
@flask_cache.cached(timeout=60 * 60)
def get_cds_product(start_date, end_date, datatake_id):
    try:
        ...
        indices = elastic_utils.get_index_name_from_interval_date(
            "cds-product", start_date, end_date
        )
        elastic = elastic_client.ElasticClient()
        results = []
        for index in indices:
            try:
                ...
                logger.error(
                    "Received error while executing query, on Elastic: %s",
                    ex,
                    exc_info=True,
                )
...
@blueprint.route(
    "/api/debug/cds-publication-trend-by-mission/<start_date>/<end_date>/<num_periods>/<mission>",
    methods=["GET"],
)
@login_required
@flask_cache.cached(timeout=60 * 60)
def get_cds_publication_trend_by_mission(start_date, end_date, num_periods, mission):
    logger.info(
        "Trend for mission %s, %s periods, from %s to %s",
        mission,
        num_periods,
        start_date,
        end_date,
    ...
    try:
        ...
        start_date = datetime.strptime(start_date, "%d-%m-%YT%H:%M:%S")
        end_date = datetime.strptime(end_date, "%d-%m-%YT%H:%M:%S")
        trend_result = elastic_publication.get_cds_publication_trend_by_mission(
            start_date, end_date, mission, num_periods
        )
        api_end_time = perf_counter()
        logger.info(
            f"API TREND for mission {mission}, start: {start_date}, end: {end_date} - Execution Time : {api_end_time - api_start_time:0.6f}"
        )

        return Response(
            json.dumps(trend_result), mimetype="application/json", status=200
        )
    except Exception as ex:
...
@blueprint.route(
    "/api/debug/cds-publication-count-by-mission/<start_date>/<end_date>/<mission>",
    methods=["GET"],
)
@login_required
@flask_cache.cached(timeout=60 * 60)
def get_cds_publication_count_by_mission(start_date, end_date, mission):
    try:
        ...
        logger.debug(
            "Get CDS Publication COUNT by mission: From %s, to %s, Mission: %s",
            start_date,
            end_date,
            mission,
        )
        start_date = datetime.strptime(start_date, "%d-%m-%YT%H:%M:%S")
        end_date = datetime.strptime(end_date, "%d-%m-%YT%H:%M:%S")
        results = elastic_publication.get_cds_publication_count_by_mission(
            start_date, end_date, mission
        )
...
@blueprint.route(
    "/api/debug/cds-publication-size-by-mission/<start_date>/<end_date>/<mission>",
    methods=["GET"],
)
@login_required
@flask_cache.cached(timeout=60 * 60)
def get_cds_publication_size_by_mission(start_date, end_date, mission):
    try:
        ...
        logger.debug(
            "Get CDS Publication SIZE by mission: From %s, to %s, Mission: %s",
            start_date,
            end_date,
            mission,
        )
        ...
        results = elastic_publication.get_cds_publication_size_by_mission(
            start_date, end_date, mission
        )
        logger.debug("Received results from cds_publication_count: %s", results)
...
@blueprint.route(
    "/api/debug/cds-publication-by-datatakes-id-and-date", methods=["POST"]
)
@login_required
@flask_cache.cached(timeout=60 * 60)
def get_cds_publication_by_datatakes_id():
    try:
        ...
        for result in results:
            ...
            if (
                datetime.strptime(result["publication_date"], "%Y-%m-%dT%H:%M:%S.%fZ")
                >= start_date
                and datetime.strptime(
                    result["publication_date"], "%Y-%m-%dT%H:%M:%S.%fZ"
                )
                <= end_date
            ):
        ...
        return Response(
            json.dumps(list_filtered, cls=db_utils.AlchemyEncoder),
            mimetype="application/json",
            status=200,
        )
    except Exception as ex:
...
@blueprint.route(
    "/api/debug/cds-publication-size-by-mission-quarter/<year>/<quarter>/<mission>",
    methods=["GET"],
)
@login_required
...
@blueprint.route(
    "/api/debug/cds-publication-count-by-mission-quarter/<year>/<quarter>/<mission>",
    methods=["GET"],
)
@login_required
@flask_cache.cached(timeout=2628288 * 3)  # 3 Months
