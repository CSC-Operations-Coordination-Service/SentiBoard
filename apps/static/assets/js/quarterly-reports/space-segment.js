...
class SpaceSegment {
    ...
    refreshAvailabilityStatus() {
        ...
        Object.keys(this.satUnavailabilities).forEach(function (ref, key) {
            ...
            if (!availabilityStatus[satellite]) {
                console.warn('Satellite' + satellite + ' not found');
                return;
            }
    ...
    successLoadDatatakes(response) {
        ...
        for (var i = 0; i < rows.length; ++i) {
            ...
            console.log("Processing record " + i + " for satellite: ", sat_unit);
            ...
            if (!this.datatakesBySatellite[sat_unit]) {
                console.warn("Initializing new array for unexpected satellite: " + sat_unit);
                this.datatakesBySatellite[sat_unit] = [];
            ...
            if (element['last_attached_ticket'] && element['completeness_status'].ACQ.percentage < 100) {
                if (!this.impactedDatatakesBySatellite[sat_unit]) {
                    this.impactedDatatakesBySatellite[sat_unit] = [];
                }
