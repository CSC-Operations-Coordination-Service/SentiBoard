    ...
    def setter(self, val):
        ...
        if isinstance(val, cls):
            ...
            try:
                ...
                raise TypeError(
                    "Property %s must be of class %s. %s was provided."
...
class _DateTimeAware(_CZMLBaseObject):
    ...
    epoch = datetime_property(
        "epoch",
        doc=""" Specifies the epoch to use for times specifies as seconds
        since an epoch. """,
    )
    nextTime = datetime_property(
        "nextTime",
        allow_offset=True,
        doc="""The time of the next sample within this interval, specified as
    ...
    previousTime = datetime_property(
        "previousTime",
        allow_offset=True,
        doc="""The time of the previous sample within this interval, specified
        ...
        if isinstance(coords, (list, tuple)):
            try:
                ...
                if len(coords) < 3:
                    ...
                    self.coords = [
                        _Coordinate(coords[1], coords[2], coords[3], coords[0])
                    ]
                elif len(coords) >= 4:
                    self.coords = []
                    for coord in grouper(coords, 4):
                        self.coords.append(
                            _Coordinate(coord[1], coord[2], coord[3], coord[0])
                        )
            except TypeError:
                self.coords = []
...
class Number(_DateTimeAware):
    ...
    def data(self):
        ...
        if (
            ("number" in data)
            and (len(data.keys()) == 1)
            and isinstance(data["number"], (int, float, str, long))
        ):
            return data["number"]
    ...
    def __init__(self, colors, num=float):
        if isinstance(colors, (list, tuple)):
            if len(colors) == 3:
                ...
                self.colors = [
                    _Color(
                        colors[1], colors[2], colors[3], colors[4], colors[0], num=num
                    )
                ]
            elif len(colors) >= 5:
                self.colors = []
                for color in grouper(colors, 5):
                    self.colors.append(
                        _Color(
                            color[1], color[2], color[3], color[4], color[0], num=num
                        )
                    )
            else:
                raise ValueError
...
class Cone(_DateTimeAware, _CZMLBaseObject):
    ...
    def __init__(self, epoch=None, nextTime=None, previousTime=None, **kwargs):
        ...
        _DateTimeAware.__init__(
            self, epoch=epoch, nextTime=nextTime, previousTime=previousTime
        )
